...
class NoteEditScreenState extends State<NoteEditScreen> {
    ...
    if (comparisonOperator != null) {
      ...
      if (parts.length == 2) {
        try {
          ...
          String message = isValid ? 'noteEditScreen.true'.tr() : 'noteEditScreen.false'.tr();
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text('noteEditScreen.errorSavingNote'.tr(args: [e.toString()]))),
          );
  ...
  void _insertAtCursor(String text) {
    final TextEditingController activeController = _showMathField ? _mathController : _contentController;
  ...
  Widget _buildMathButton(String symbol, ThemeProvider themeProvider) {
    return Padding(
      ...
      child: ElevatedButton(
        onPressed: () => _insertAtCursor(symbol),
        ...
        child: Text(
          symbol,
          style: TextStyle(fontSize: 20, color: themeProvider.accentColor),
        ),
      ),
    );
  }
